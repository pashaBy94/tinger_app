...
class HtmlTelegramBot extends TelegramBot {
  ...
  async sendText(text, options = {}) {
    return await super.sendMessage(this.singleChatId, text, {
      parse_mode: "HTML",
      ...options,
    });
  }
  ...
  async sendTextButtons(text, buttons, options = {}) {
    const keyboard = Object.keys(buttons).map((key) => [
      { text: buttons[key], callback_data: key },
    ]);
  ...
  async sendImage(name) {
    ...
    return await super.sendPhoto(
      this.singleChatId,
      fs.createReadStream(`./resources/images/${name}.jpg`),
  ...
  async editText(myMessage, newText) {
    await super.editMessageText(newText, {
      chat_id: myMessage.chat.id,
      message_id: myMessage.message_id,
    });
  }
